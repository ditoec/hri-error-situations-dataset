...
BAG_DIRECTORY=$1
for bag in $(find ${BAG_DIRECTORY} -name '*dp.bag'); do
	echo "Processing bag file ${bag}"

	INPUT_DATASET_FILE="${bag}"
	OUTPUT_TEXT_FILE="${bag%.bag}.csv"

	if [ -f $OUTPUT_TEXT_FILE ]; then
		echo "Output csv file ${OUTPUT_TEXT_FILE} already found"
		echo "Skipping analysis for bag file ${INPUT_DATASET_FILE}"
		continue
	fi

	python data2csv.py $INPUT_DATASET_FILE -o $OUTPUT_TEXT_FILE
	if ! [ -f $OUTPUT_TEXT_FILE ]; then
